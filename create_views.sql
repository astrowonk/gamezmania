CREATE VIEW if not exists rounds_view as
select
	r.*,
	case
...
from
	rounds r
	left join player_names p on r.player = p.player_id
...
CREATE VIEW if not exists bids_view as
select
	r.*,
	case
		when p.player_name is null then r.player
...
select
	player,
	max(file_name) as last_played_game
from
	scores_view sv
group by
	player
...
CREATE VIEW if not exists lucky_or_good as
select
	*
from
	(
		select
			...
			svv.player_name,
			last_played_game,
			avg(made_bid - prediction) as made_bid_minus_prediction,
			count(made_bid) as game_count
		from
			(
				select
					player,
			...
			left join predictions p on p.unique_hash = svv.unique_hash
			and p.player == svv.player
			left join last_played lp on svv.player = lp.player
		group by
			svv.player,
			svv.player_name
	)
...
CREATE VIEW if not exists predictions as
select
	player,
	unique_hash,
...
CREATE VIEW if not exists predictions_detail_view as
select
	case
	...
	is_trump
from
	predictions_detail pd
	left join player_names pn on pn.player_id = pd.player
...
from
	(
		select
			round,
			(sum(set_bid) / avg(player_count)) as set_pct,
			file_name,
			player_count
		from
			(
				...
				from
					scores_view sv
					left join player_count pc on pc.file_name = sv.file_name
			)
		group by
			file_name,
			round
