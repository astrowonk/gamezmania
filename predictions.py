...
def make_delta_columns(df, column1, column2):
    ...
    """
    df.loc[:, column1 + '_minus_' + column2] = (
        df.loc[:, column1] - df.loc[:, column2]
    ).astype(float)
...
def make_ratio_columns(df, column1, column2):
    ...
    df.loc[:, col_name] = (df.loc[:, column1] / df.loc[:, column2]).astype(float)
    return col_name
...
class PredictBid:
    ...
    def prep_data(self):
        score_data_training = self.df_scores.set_index(['unique_hash', 'player', 'round'])[
            [
                'taken_minus_bid',
                'bid_div_total_cards',
                'total_cards',
        ...
        double_deck_map = (
            score_data_training.reset_index().groupby(['unique_hash'])['total_cards'].max()
        )
        player_count = (
            score_data_training.reset_index().groupby(['unique_hash'])['player'].nunique()
        )

        deck_map = (double_deck_map * player_count) // 52

        trump_data = pd.get_dummies(
            self.df_rounds.query('is_trump == True')['card_rank'], prefix='trump'
        ).astype('int')

        singleton_data = pd.get_dummies(
            self.df_rounds.query('n_cards_suit_round == 1')['card_rank'], prefix='singles'
        ).astype('int')
        score_data_training['made_bid'] = (score_data_training['taken_minus_bid'] == 0).astype(
            int
        )
        ...
        self.final_training = (
            self.final_training.set_index(['unique_hash', 'round', 'player'])
            .join(dealer_map)
            .reset_index()
            .query('total_cards >= 4')
        )
            ...
            try:
                hashes = {
                    ...
                    for x in con.execute(
                        text(f'select distinct (unique_hash) from {table_name};')
                    )
                }
        ...
        with engine.connect() as con:
            try:
                hashes = {
                    x[0]
                    for x in con.execute(
                        text(f'select distinct (unique_hash) from predictions_detail;')
                    )
                }
            except OperationalError:
                print('hash fail')
    ...
    def make_alt_bids(one_player: pd.DataFrame, xgb, cols_train):
        one_player['bid'] = (
            one_player['bid_div_total_cards'] * one_player['total_cards']
        ).astype(int)
        rec = one_player.iloc[0]
        other_bids = rec['total_bid_minus_total_cards'] + rec['total_cards'] - rec['bid']
        ...
        df['total_bid_minus_total_cards'] = other_bids - df['total_cards'] + bid_range
    ...
    def train(self, unique_hash, upload=True):
        ...
        xgb.fit(
            X_train.drop(columns=['taken_minus_bid'], errors='ignore'),
            y_train,
            eval_set=[(X_test, y_test)],
        )
        if upload and not test_data.empty:
            test_data['prediction'] = xgb.predict_proba(test_data[COLS_TRAIN])[:, 1]

            max_prediction = self.make_all_alt_bids(test_data, xgb, cols_train=COLS_TRAIN)
        res = ''
        if upload:
            res += self._upload(test_data, 'predictions_detail', unique_hash) + '\n'
            res += self._upload(max_prediction, 'prediction_alt_bids_detail', unique_hash)
            return res
        else:
            return xgb
...
class PlayerInfluence:
    ...
    def process_data(self):
        ...
        temp_data = pd.get_dummies(final_scores, columns=['player_name'], prefix='player_name')
        mylist = [x for x in temp_data.columns if x.startswith('player_name')]
        fitting_data = temp_data.groupby('unique_hash')[mylist].sum()
        temp_fit_data = final_scores.set_index('unique_hash').join(fitting_data)
        temp_fit_data = final_scores.set_index('unique_hash').join(fitting_data)
        temp_fit_data['max_score'] = temp_fit_data.groupby('unique_hash')['points'].transform(
            max
        )
        temp_fit_data['norm_score'] = temp_fit_data['points'] / temp_fit_data['max_score']
        temp_fit_data['win_flag'] = (
            temp_fit_data['points'] == temp_fit_data['max_score']
        ).astype(int)
        temp_fit_data['score_rank'] = temp_fit_data.groupby('unique_hash')['points'].rank(
            ascending=False
        )
        temp_fit_data['n_players'] = temp_fit_data.groupby('unique_hash')['player'].transform(
            'nunique'
        )
        temp_fit_data.to_sql('linear_model_data', con=con, if_exists='replace')

    def get_linear_fit_menu_data(self, user, min_count=10, n=150, return_users=False):
        con = create_engine(f'sqlite:///{self.db_name}')
        ...
        cols = [x for x in temp_fit_data.columns if x.startswith('player_name_')]
        other_cols = [x for x in temp_fit_data.columns if not x.startswith('player_name_')]
        col_series = temp_fit_data[cols].sum() > min_count

        qualified_user_variables = {
            x: y for x, y in temp_fit_data[cols].sum().to_dict().items() if y > min_count
        }
    ...
    def build_model(self, thedata, name, dependent, fitting_variables):
        ...
        fitting_variables = [x for x in fitting_variables if x != f'player_name_{name}']
